...
{
	if (nSize <= nIndex)//索引从0开始，如果大小小于等于索引说明没有数据
	{
		return false;
	}
...
int CBufferWriter::nSend(int sockfd)
{
	...
	do
	{
		...
		nBytesSend = ::send(sockfd, pkt.data.get() + pkt.writeIndex, (int)(pkt.size - pkt.writeIndex), 0);
		...
		else if (nBytesSend < 0)
		{
			...
			if (nError == WSAEWOULDBLOCK || nError == WSAEINPROGRESS || nError == 0)
			{
				nBytesSend = 0;
			}
		}
